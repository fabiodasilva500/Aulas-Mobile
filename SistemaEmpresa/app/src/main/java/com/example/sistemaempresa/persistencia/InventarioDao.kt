...
class InventarioDao(context: Context) :
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_NAME_INVENTARIO)
        onCreate(db)
    }
    ...
    fun inserirDados(nome: String, descricao: String,
                     localidade: String, numeroDisponivel: String) {
        val db = this.writableDatabase
        val contentValues = ContentValues()
        contentValues.put(COL_2, nome)
        contentValues.put(COL_3, descricao)
        contentValues.put(COL_4, localidade)
        contentValues.put(COL_5, numeroDisponivel)


        db.insert(TABLE_NAME_INVENTARIO, null, contentValues)
    }
    ...
    fun alterarDados(id:String, nome: String, descricao: String,
                     localidade: String, numeroDisponivel: String): Boolean {
        val db = this.writableDatabase
        val contentValues = ContentValues()
        ...
        db.update(TABLE_NAME_INVENTARIO, contentValues, "ID = ?", arrayOf(id))
        return true
    }
    ...
    fun deletarDados(id: String): Int {
        val db = this.writableDatabase
        return db.delete(TABLE_NAME_INVENTARIO, "ID = ?", arrayOf(id))
    }
    ...
    val allData: Cursor
        get() {
            val db = this.writableDatabase
            val res = db.rawQuery("SELECT * FROM " + TABLE_NAME_INVENTARIO, null)
            return res
        }
    ...
    companion object {
        ...
        val TABLE_NAME_INVENTARIO = "Inventario"
        val COL_1 = "ID"
        val COL_2 = "NOME"
        val COL_3 = "DESCRICAO"
        val COL_4 = "LOCALIDADE"
        val COL_5 = "NUMERODISPONIVEL"
    }
}
